...
describe(NAME, function () {
  async function setup() {
    ...
    const VulnerableDeFiFactory = await ethers.getContractFactory(
      "VulnerableDeFiContract"
    );
    const vulnerableDeFiContract = await VulnerableDeFiFactory.deploy(
  ...
  describe("exploit", async function () {
    ...
    before(async function () {
      ({ readOnlyContract, vulnerableDeFiContract, attackerWallet } =
    ...
    after(async function () {
      ...
      expect(await vulnerableDeFiContract.lpTokenPrice()).to.be.equal(
        0,
        "snapshotPrice should be zero"
      );
      expect(
        await ethers.provider.getTransactionCount(attackerWallet.address)
      ).to.lessThan(3, "must exploit two transactions or less");
    });
  });
});
